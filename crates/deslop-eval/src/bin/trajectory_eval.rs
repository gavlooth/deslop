...
fn path(value: Option<std::ffi::OsString>, label: &str) -> Result<PathBuf> {
    value.map(PathBuf::from).with_context(|| format!("missing {label}"))
}
...
fn main() -> Result<()> {
    ...
    let mode = args.next().context(
        "usage: trajectory-eval <replay|candidates|adapt-opencode> ...",
    )?;
    match mode.to_string_lossy().as_ref() {
        "replay" => {
            let input = path(args.next(), "trajectory path")?;
            if args.next().is_some() { bail!("unexpected replay argument"); }
            let trajectory = import_trajectory(&fs::read(&input).with_context(|| format!("read {}", input.display()))?)?;
            let evidence = replay_trajectory(&trajectory)?;
            println!("trajectory replay OK: {} events, final {}", evidence.applied_events.len(), evidence.final_hash);
        }
        "candidates" => {
            let input = path(args.next(), "trajectory path")?;
            if args.next().is_some() { bail!("unexpected candidates argument"); }
            let trajectory = import_trajectory(&fs::read(&input).with_context(|| format!("read {}", input.display()))?)?;
        ...
        "adapt-opencode" => {
            ...
            let spdx = args.next().context("missing SPDX license identifier")?.to_string_lossy().into_owned();
            let workspace_root = args.next().map(PathBuf::from);
            if args.next().is_some() { bail!("unexpected adapt-opencode argument"); }
            let base = read_snapshot(&base_path)?;
            let final_snapshot = read_snapshot(&final_path)?;
            let license = LicenseGate { approved: true, spdx: Some(spdx), source_uri: Some("opencode export".to_string()) };
            ...
            println!("OpenCode trajectory OK: {} events -> {}", trajectory.events.len(), output.display());
        }
