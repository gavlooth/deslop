...
fn composed_failure_preserves_sources_and_creates_no_backups() -> Result<()> {
    ...
    let patches: Vec<_> = batch.work_orders.iter()
    ...
    let options = VerifyOptions {
        root: temp.path().into(), scope: None,
        check_cmd: Some("grep -q 'return 1;' a.rs b.rs".into()),
        coverage: CoverageConfig::Disabled, mutation: MutationConfig::Disabled,
        characterization_tests: vec![], allow_non_removable: true,
    };
    let sandbox_available = Command::new("bwrap")
        .args(["--unshare-all", "--ro-bind", "/", "/", "--proc", "/proc", "--dev", "/dev", "--", "/usr/bin/true"])
        .output().is_ok_and(|output| output.status.success());
    for patch in &patches {
        let result = verify_patches(std::slice::from_ref(patch), &options)?;
        if sandbox_available {
            ensure!(result.failed_count() == 0, "individual candidate: {result:?}");
        } else {
            ensure!(result.failed_count() == 1, "unavailable sandbox must reject");
        }
    }
