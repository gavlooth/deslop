...
fn public_export_replays_but_does_not_grant_write_authority() -> Result<()> {
    ...
    let final_snapshot = Snapshot::new("final", BTreeMap::from([("sample.rs".into(), verbose.into())]))?;
    ...
    let trajectory = import_opencode_export(&export, base, final_snapshot, LicenseGate {
        spdx: Some("MIT".into()), approved: true,
    ...
    let order = proposals.work_orders.iter()
        .find(|order| order.region.text.contains("return 1;")).expect("actual source proposal");
    let patch = Patch {
        schema: "deslop.patch/3".into(), workorder_id: order.id.clone(),
        revision_guard: workorder_revision_guard(order).clone(),
        proposal_context: order.proposal_context.clone(),
        replacement: order.region.text.replace("return 1;", "1"), by: "engineering-fixture".into(),
    };
    let mut options = VerifyOptions {
        root: root.path().into(), scope: None, check_cmd: None,
        coverage: CoverageConfig::Disabled, mutation: MutationConfig::Disabled,
        characterization_tests: vec![], allow_non_removable: false,
    };
    ensure!(verify_and_apply_candidate(&candidates[0], std::slice::from_ref(&patch), &options, false).is_err());
